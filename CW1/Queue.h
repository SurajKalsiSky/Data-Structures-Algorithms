...
public:
	Queue(int size) {
		...
		data = new int[size]; // initialise array to hold the queue (with allocated size)
		front = 0;  // initialise front pointer
		rear = -1; // initialise rear pointer
		count = 0; // initialise count variable
	}
	int PeakFront() { // returns front element of the queue
		return data[front];
	}
	...
	void Insert(int item) { // insert an item into the queue
		if (IsFull()) {
			return; // do not insert if queue is full
		}
	...
	int Remove() { // remove an item from the queue
		if (!IsEmpty()) {
			int frontValue = PeakFront(); // get item at front of queue
